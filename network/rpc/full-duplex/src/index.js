...
// };
//
// const is_receive_complete = buffer => {
...
//
//     while (buffer && buffer.length && (package_length = is_receive_complete(buffer))) {
//         request_buffer = buffer.slice(0, package_length);
//         buffer = buffer.slice(package_length);
//
//         const {seq, result} = decode(request_buffer);
//
...
console.log('NodeJS《第十六课: rpc 基于 tcp/udp 协议的网络请求,二进制数据传输-full-duplex 全双工通信》部分课程已完美的回溯完成!如需要检测执行结果,将以上代码注释解开即可!');
